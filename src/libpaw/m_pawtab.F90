...
!Integer scalars
  ...
  integer :: has_x_nabla
   ! if 1, onsite matrix elements of the x_nabla operator are allocated.
   ! if 2, onsite matrix elements of the x_nabla operator are computed and stored.

  integer :: has_dipole
   ! if 1, onsite matrix elements of the dipole operator are allocated.
   ! if 2, onsite matrix elements of the dipole operator are computed and stored.
...
!Real (real(dp)) arrays
  ...
  real(dp), allocatable :: x_nabla_ij(:,:,:,:)
   ! nabla_ij(3,3,lmn_size,lmn_size)
   ! Onsite matrix elements <phi| x \nabla_y|phj>-<tphi|x \nabla_y|tphj>

  real(dp), allocatable :: dipole_ij(:,:,:)
   ! nabla_ij(3, lmn_size,lmn_size)
   ! Onsite matrix elements <phi| x |phj>-<tphi|x |tphj>
...
! *************************************************************************
 ...
 if (allocated(Pawtab%x_nabla_ij))  then
  LIBPAW_DEALLOCATE(Pawtab%x_nabla_ij)
end if
if (allocated(Pawtab%dipole_ij))  then
  LIBPAW_DEALLOCATE(Pawtab%dipole_ij)
end if
...
&                              has_vhtnzc,has_nabla,has_x_nabla,has_dipole, has_nablaphi,has_shapefncg,has_vminushalf,has_wvl)
...
!Arguments ------------------------------------
 ...
 integer,intent(in),optional :: has_nabla,has_x_nabla,has_dipole, has_nablaphi,has_shapefncg,has_wvl
 type(pawtab_type),intent(inout) :: Pawtab
...
! *************************************************************************
 ...
 if (present(has_x_nabla))     Pawtab%has_x_nabla=has_x_nabla
 if (present(has_dipole))     Pawtab%has_dipole=has_dipole
...
!Arguments ------------------------------------
 ...
 integer,intent(in),optional :: has_nabla,has_x_nabla,has_dipole, has_nablaphi,has_shapefncg,has_vminushalf,has_wvl
 type(pawtab_type),intent(inout) :: Pawtab(:)
...
! *************************************************************************
 ...
 do ii=1,nn
   ...
   Pawtab(ii)%has_x_nabla   =0
   Pawtab(ii)%has_dipole   =0
   ...
   if (present(has_x_nabla))     Pawtab(ii)%has_x_nabla=has_x_nabla
   if (present(has_dipole))     Pawtab(ii)%has_dipole=has_dipole
...
&  ' ==================================== ',ch10
 ...
 do ityp=1,ntypat
  ...
  call wrtout(my_unt,msg,my_mode)
  write(msg,'(a,i4)')'  Has x_nabla ...................................... ',Pawtab(ityp)%has_x_nabla
  call wrtout(my_unt,msg,my_mode)
  write(msg,'(a,i4)')'  Has dipole ...................................... ',Pawtab(ityp)%has_dipole
  call wrtout(my_unt,msg,my_mode)
...
!scalars
 ...
 integer :: siz_x_nabla_ij,siz_dipole_ij
...
!-------------------------------------------------------------------------
!  basis_size,has_coretau,has_fock,has_kij,has_shapefncg,has_nabla,has_x_nabla, has_dipole,has_nablaphi,has_tproj
...
!-------------------------------------------------------------------------
   ...
   siz_x_nabla_ij=0; siz_dipole_ij=0; 
   ...
   if (full_broadcast) then
     ...
     if (allocated(pawtab%nabla_ij)) then
       ...
       if (siz_nabla_ij/=3*pawtab%lmn_size**2) msg=trim(msg)//' nabla_ij'
       nn_dpr=nn_dpr+siz_nabla_ij
     end if
     if (allocated(pawtab%x_nabla_ij)) then
       siz_x_nabla_ij=size(pawtab%x_nabla_ij)             !(3,3,lmn_size,lmn_size)
       if (siz_x_nabla_ij/=9*pawtab%lmn_size**2) msg=trim(msg)//' x_nabla_ij'
       nn_dpr=nn_dpr+siz_x_nabla_ij
    end if
    if (allocated(pawtab%dipole_ij)) then
      siz_dipole_ij=size(pawtab%dipole_ij)             !(3,lmn_size,lmn_size)
      if (siz_dipole_ij/=3*pawtab%lmn_size**2) msg=trim(msg)//' dipole_ij'
      nn_dpr=nn_dpr+siz_dipole_ij
    end if
     if (allocated(pawtab%phiphj)) then
...
!Integers (read from psp file)
   ...
   list_int(ii)=pawtab%has_x_nabla  ;ii=ii+1
   list_int(ii)=pawtab%has_dipole  ;ii=ii+1
...
!Sizes of arrays
     ...
     list_int(ii)=siz_x_nabla_ij  ;ii=ii+1
     list_int(ii)=siz_dipole_ij  ;ii=ii+1
...
!Integers (read from psp file)
   ...
   pawtab%has_x_nabla=list_int(ii)  ;ii=ii+1
   pawtab%has_dipole=list_int(ii)  ;ii=ii+1
...
!Sizes of arrays
     ...
     siz_x_nabla_ij=list_int(ii)  ;ii=ii+1
     siz_dipole_ij=list_int(ii)  ;ii=ii+1
...
!Reals arrays
     ...
     if (siz_x_nabla_ij>0) then
      list_dpr(ii:ii+siz_x_nabla_ij-1)=reshape(pawtab%x_nabla_ij,(/siz_x_nabla_ij/))
      ii=ii+siz_x_nabla_ij
    end if
    if (siz_dipole_ij>0) then
      list_dpr(ii:ii+siz_dipole_ij-1)=reshape(pawtab%dipole_ij,(/siz_dipole_ij/))
      ii=ii+siz_dipole_ij
    end if
...
!Reals arrays
    ...
    if (siz_x_nabla_ij>0) then
      LIBPAW_ALLOCATE(pawtab%x_nabla_ij,(3,3,pawtab%lmn_size,pawtab%lmn_size))
      pawtab%x_nabla_ij=reshape(list_dpr(ii:ii+siz_x_nabla_ij-1),(/3,3,pawtab%lmn_size,pawtab%lmn_size/))
      ii=ii+siz_x_nabla_ij
    end if
    if (allocated(pawtab%dipole_ij)) then
      LIBPAW_DEALLOCATE(pawtab%dipole_ij)
    ...
    if (siz_dipole_ij>0) then
      LIBPAW_ALLOCATE(pawtab%dipole_ij,(3,pawtab%lmn_size,pawtab%lmn_size))
      pawtab%dipole_ij=reshape(list_dpr(ii:ii+siz_dipole_ij-1),(/3,pawtab%lmn_size,pawtab%lmn_size/))
      ii=ii+siz_dipole_ij
    end if
     if (allocated(pawtab%phiphj)) then
       LIBPAW_DEALLOCATE(pawtab%phiphj)
