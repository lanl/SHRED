...
& mpi_atmtab,comm_atom,vpotzero,hyb_mixing,hyb_mixing_sr) ! optional arguments
...
!scalars
 ...
 integer :: iatom,iatom_tot,idum,ierr,ii,iq,iq0_dij,iq0_rhoij
...
!arrays
 ...
 logical,allocatable :: lmselect_cur(:),lmselect_tmp(:)
 real(dp) :: mpiarr(7)
 real(dp),allocatable :: dijfock_vv(:,:),dijfock_cv(:,:)
 ...
 real(dp),allocatable :: rho(:),rho1(:,:,:),rho1xx(:,:,:)
 real(dp),allocatable :: tau1(:,:,:),ttau1(:,:,:), trho1(:,:,:)
...
!  Allocations of "on-site" densities
   LIBPAW_ALLOCATE(rho1 ,(cplex*mesh_size,lm_size,nspden))
   LIBPAW_ALLOCATE(trho1,(cplex*mesh_size,lm_size,nspden))
   LIBPAW_ALLOCATE(nhat1,(cplex*mesh_size,lm_size,nspden*usenhat))
   rho1(:,:,:)=zero;trho1(:,:,:)=zero;nhat1(:,:,:)=zero
   if (usekden==1) then
     LIBPAW_ALLOCATE(tau1 ,(cplex*mesh_size,lm_size,nspden))
     LIBPAW_ALLOCATE(ttau1,(cplex*mesh_size,lm_size,nspden))
   end if
   LIBPAW_ALLOCATE(lmselect_cur,(lm_size))
   lmselect_cur(:)=.true.
...
!  Store some usefull quantities
   ...
   if (itypat/=itypat0) then
     LIBPAW_ALLOCATE(one_over_rad2,(mesh_size))
...
!  Need to allocate vxc1 in particular cases
   if (pawspnorb>0.and.ipert==0.and.option==2.and. &
...
!    These should already be allocated in paw_an_init!
     if (allocated(paw_an(iatom)%vxc1))  then
       LIBPAW_DEALLOCATE(paw_an(iatom)%vxc1)
     end if
     if (pawxcdev==0)then
       LIBPAW_ALLOCATE(paw_an(iatom)%vxc1,(cplex*mesh_size,paw_an(iatom)%angl_size,nspden))
     else
       LIBPAW_ALLOCATE(paw_an(iatom)%vxc1,(cplex*mesh_size,lm_size,nspden))
     end if
...
!  Temporary storage
   ...
   if (pawxcdev/=0) then
     LIBPAW_ALLOCATE(vxc_tmp,(cplex*mesh_size,lm_size,nspden))
     if (need_kxc) then
       LIBPAW_ALLOCATE(kxc_tmp,(mesh_size,lm_size,nkxc1))
     end if
     if (need_k3xc) then
   ...
   if (pawxcdev==0) then
     LIBPAW_ALLOCATE(vxc_tmp,(cplex*mesh_size,pawang%angl_size,nspden))
     vxc_tmp(:,:,:)=zero
     if (need_kxc) then
       LIBPAW_ALLOCATE(kxc_tmp,(mesh_size,pawang%angl_size,nkxc1))
     end if
     if (need_k3xc) then
       LIBPAW_ALLOCATE(k3xc_tmp,(mesh_size,pawang%angl_size,nk3xc1))
     end if
     if (need_vxctau) then
       LIBPAW_ALLOCATE(vxctau_tmp,(cplex*mesh_size,pawang%angl_size,nspden))
       vxctau_tmp(:,:,:)=zero
     end if
   end if
   idum=0
   if (.not.allocated(vxc_tmp))  then
     LIBPAW_ALLOCATE(vxc_tmp,(0,0,0))
   end if
   if (.not.allocated(kxc_tmp))  then
     LIBPAW_ALLOCATE(kxc_tmp,(0,0,0))
   end if
   if (.not.allocated(k3xc_tmp))  then
     LIBPAW_ALLOCATE(k3xc_tmp,(0,0,0))
   end if
   if (.not.allocated(vxctau_tmp))  then
     LIBPAW_ALLOCATE(vxctau_tmp,(0,0,0))
   end if
...
&       pawang,pawrad(itypat),trho1,0,2*usexcnhat,vxc_tmp,xclevel,xc_denpos)
   ...
   LIBPAW_DEALLOCATE(vxc_tmp)
   if (usekden==1) then
     LIBPAW_DEALLOCATE(vxctau_tmp)
   end if
   LIBPAW_DEALLOCATE(kxc_tmp)
   LIBPAW_DEALLOCATE(k3xc_tmp)

!  ===== Compute first part of local exact-exchange energy term =====
!  ===== Also compute corresponding potential                   =====
!  ==================================================================

   if (pawtab(itypat)%useexexch/=0.and.ipert==0) then
...
!    ===== Re-compute Exc1 and Vxc1; for local exact-exchange, this is done in GGA only
     LIBPAW_ALLOCATE(vxc_tmp,(mesh_size,lm_size,nspden))
     LIBPAW_ALLOCATE(kxc_tmp,(mesh_size,lm_size,nkxc1))
...
&     rho1xx,0,0,vxc_tmp,xclevel,xc_denpos)
   ...
   if (itypat/=itypat0) then
     LIBPAW_DEALLOCATE(one_over_rad2)
   end if

   LIBPAW_DEALLOCATE(lmselect_cur)
...
!    of Kresse and Joubert PRB 59 1758 (1999) [[cite:Kresse1999]]
   ...
   if ((pawspnorb>0.and.ipert==0).or.keep_vhartree) then

     !In the first clause case, would it not be simpler just to turn on has_vhartree?
     if (.not. allocated(paw_an(iatom)%vh1)) then
       LIBPAW_ALLOCATE(paw_an(iatom)%vh1,(cplex*mesh_size,1,1))
     end if
     if (.not. allocated(paw_an(iatom)%vht1)) then
       LIBPAW_ALLOCATE(paw_an(iatom)%vht1,(cplex*mesh_size,1,1))
     end if
     LIBPAW_ALLOCATE(rho,(mesh_size))
     LIBPAW_ALLOCATE(vh,(mesh_size))
...
!  ==========================================================
   ...
   if (usepawu/=0.and.usepawu<10.and.option/=1) then
...
!      PAW+U Dij computation from eU_ijkl
       LIBPAW_CHECK(ndij/=4,'ndij=4 not yet available!')
       !First, compute DijU
...
&                          pawrhoij(iatom),pawtab(itypat))
       ...
       if (option/=1) then
         ...
         if (pawu_dblec==PAWU_FLL.and.ipert==0) then
           LIBPAW_CHECK(qphase==1,'BUG in pawdenpot: qphase should be 1 for Dble-C FLL term!')
...
!  =====================================================================
   ...
   if (any(abs(nucdipmom(:,iatom))>tol8).and.ipert==0) then

     LIBPAW_CHECK(cplex_rhoij==2,'BUG in pawdenpot: rhoij must be complex for ND moments!')
     LIBPAW_CHECK(qphase==1,'BUG in pawdenpot: qphase should be 1 for ND moments!')
...
!  ==========================================================
   ...
   if (pawspnorb>0.and.ipert==0) then
...
!  ======================================================================
   ...
   if (pawtab(itypat)%useexexch/=0.and.ipert==0) then
     ...
     LIBPAW_CHECK(paw_ij(iatom)%nspden/=4,'BUG in pawdenpot: Local ex-exch. not implemented for nspden=4!')
     if (option<2) then
...
!      Fock contribution to Dij
       LIBPAW_ALLOCATE(dijfock_vv,(cplex_dij*qphase*lmn2_size,ndij))
       LIBPAW_ALLOCATE(dijfock_cv,(cplex_dij*qphase*lmn2_size,ndij))
...
!  Deallocate densities
   LIBPAW_DEALLOCATE(rho1)
   LIBPAW_DEALLOCATE(trho1)
   LIBPAW_DEALLOCATE(nhat1)
   if (usekden==1) then
     LIBPAW_DEALLOCATE(tau1)
     LIBPAW_DEALLOCATE(ttau1)
   end if
...
!  Deallocate potentials
   if (.not.keep_vhartree) then
     ...
     if (allocated(paw_an(iatom)%vh1)) then
       LIBPAW_DEALLOCATE(paw_an(iatom)%vh1)
     end if
   end if
   if (temp_vxc) then
...
!Various inits
 ...
 if (present(one_over_rad2)) then
   ...
   LIBPAW_ALLOCATE(one_over_rad2_,(mesh_size))
...
!  ----- End loop over spin components
 ...
 if (.not.present(one_over_rad2))  then
   LIBPAW_DEALLOCATE(one_over_rad2_)
 end if
...
!Non-collinear case: need a temporary rhoij
 if (nspden_rhoij==4.and.nspden_dij==4) then
   cplex_rhoij=2
   LIBPAW_ALLOCATE(rhoij,(2*lmn2_size,4))
 else
...
#else
 LIBPAW_ALLOCATE(dij_2D,(size_dij,1))
