...
!!
!! COPYRIGHT
!! Copyright (C) 2018-2021 ABINIT group (BA,FJ,MT)
...
!    --------------------------------------------------
     if (allocated(pawtab(itypat)%lnproju)) then
       LIBPAW_DEALLOCATE(pawtab(itypat)%lnproju)
     end if
     LIBPAW_ALLOCATE(pawtab(itypat)%lnproju,(pawtab(itypat)%nproju))
     icount=0
     do ilmn=1,lmn_size
     ...
     if (allocated(pawtab(itypat)%klmntomn)) then
       LIBPAW_DEALLOCATE(pawtab(itypat)%klmntomn)
     end if
     LIBPAW_ALLOCATE(pawtab(itypat)%klmntomn,(4,lmn2_size))
     do jlmn=1,lmn_size
       jl= indlmn(1,jlmn)
       j0lmn=jlmn*(jlmn-1)/2
       do ilmn=1,jlmn
...
&       '                     In the other case, choose dmatpuopt=2'
     ...
     if (allocated(pawtab(itypat)%ph0phiint)) then
       LIBPAW_DEALLOCATE(pawtab(itypat)%ph0phiint)
     end if
     if (allocated(pawtab(itypat)%zioneff)) then
       LIBPAW_DEALLOCATE(pawtab(itypat)%zioneff)
     end if
     LIBPAW_ALLOCATE(pawtab(itypat)%ph0phiint,(pawtab(itypat)%nproju))
     LIBPAW_ALLOCATE(pawtab(itypat)%zioneff,(pawtab(itypat)%nproju))
...
&         '  pawpuxinit: zioneff=',pawtab(itypat)%zioneff(icount)
     ...
     if (allocated(pawtab(itypat)%phiphjint)) then
       LIBPAW_DEALLOCATE(pawtab(itypat)%phiphjint)
     end if
     LIBPAW_ALLOCATE(pawtab(itypat)%phiphjint,(pawtab(itypat)%ij_proj))
...
!      ---------------------------------------------
       if (allocated(pawtab(itypat)%vee)) then
         LIBPAW_DEALLOCATE(pawtab(itypat)%vee)
       end if
       sz1=2*lpawu+1
       LIBPAW_ALLOCATE(pawtab(itypat)%vee,(sz1,sz1,sz1,sz1))
...
!      ---------------------------------------------
       ...
       if (compute_euijkl) then
         if (allocated(pawtab(itypat)%euijkl)) then
           LIBPAW_DEALLOCATE(pawtab(itypat)%euijkl)
         end if
         LIBPAW_ALLOCATE(pawtab(itypat)%euijkl,(2,2,lmn_size,lmn_size,lmn_size,lmn_size))
         ...
         if (abs(usepawu)==1.or.abs(usepawu)==4) then ! Only for FLL
           if (allocated(pawtab(itypat)%euij_fll)) then ! allocate euij_fll for FLL
             LIBPAW_DEALLOCATE(pawtab(itypat)%euij_fll)
           end if
           LIBPAW_ALLOCATE(pawtab(itypat)%euij_fll,(lmn2_size))
...
!      ---------------------------------------------
       if (allocated(pawtab(itypat)%fk)) then
         LIBPAW_DEALLOCATE(pawtab(itypat)%fk)
       end if
       LIBPAW_ALLOCATE(pawtab(itypat)%fk,(6,4))
...
!      ---------------------------------------------
       if (allocated(pawtab(itypat)%vex)) then
         LIBPAW_DEALLOCATE(pawtab(itypat)%vex)
       end if
       sz1=2*lexexch+1
       LIBPAW_ALLOCATE(pawtab(itypat)%vex,(sz1,sz1,sz1,sz1,4))
...
!-----------------------------------------------------
 ...
 LIBPAW_ALLOCATE(n12_sig,(cplex_occ))
 LIBPAW_ALLOCATE(n34_msig,(cplex_occ))
 LIBPAW_ALLOCATE(n34_sig,(cplex_occ))
...
&               - noccmmp(2,m11,m21,ispden)*n34_sig(2) *(pawtab%vee(m11,m31,m21,m41)-pawtab%vee(m11,m31,m41,m21))
 ...
 LIBPAW_DEALLOCATE(n12_sig)
 LIBPAW_DEALLOCATE(n34_msig)
 LIBPAW_DEALLOCATE(n34_sig)
...
!If needed, store dmatpu in suitable format in tmp_noccmmp
 if (usepawu/=0.and.impose_dmat/=0) then
   iatpawu=0
   LIBPAW_ALLOCATE(tmp_noccmmp,(natom))
   do iatom_tot=1,natom
     ...
     if (lpawu/=-1) then
       iatpawu=iatpawu+1
       if (ndij/=4) then
         LIBPAW_ALLOCATE(tmp_noccmmp(iatom_tot)%value,(cplex_dij,2*lpawu+1,2*lpawu+1,nsppol))
         tmp_noccmmp(iatom_tot)%value(1,1:2*lpawu+1,1:2*lpawu+1,1:nsppol)=&
&         dmatpawu(1:2*lpawu+1,1:2*lpawu+1,1:nsppol,iatpawu)
       else
         LIBPAW_ALLOCATE(tmp_noccmmp(iatom_tot)%value,(cplex_dij,2*lpawu+1,2*lpawu+1,ndij))
         ...
         if(limp==0) then ! default reading
           ...
           LIBPAW_ALLOCATE(noccmmp_ylm,(2*lpawu+1,2*lpawu+1,ndij))
           noccmmp_ylm=czero
           LIBPAW_ALLOCATE(noccmmp_slm,(2*lpawu+1,2*lpawu+1,ndij))
           noccmmp_slm=czero
           LIBPAW_ALLOCATE(noccmmp_jmj,(2*(2*lpawu+1),2*(2*lpawu+1)))
           noccmmp_jmj=czero
...
&             pawprtvol,std_out,wrt_mode) !  optspin=1: up spin are first
           ...
           LIBPAW_DEALLOCATE(noccmmp_ylm)
           LIBPAW_DEALLOCATE(noccmmp_jmj)
           LIBPAW_DEALLOCATE(noccmmp_slm)
         end if
       end if
     end if
   end do
...
!      Loop over spin components
       LIBPAW_ALLOCATE(noccmmptemp,(cplex_dij,2*lcur+1,2*lcur+1,ndij))
       noccmmptemp(:,:,:,:)=zero
       if(ndij==4)  then
         LIBPAW_ALLOCATE(noccmmp2,(cplex_dij,2*lcur+1,2*lcur+1,ndij))
       end if
       if(ndij==4)  then
         if(allocated(nocctot2)) then
           LIBPAW_DEALLOCATE(nocctot2)
         end if
         LIBPAW_ALLOCATE(nocctot2,(ndij))
       end if
       do ispden=1,ndij
         jrhoij=1
...
!      Transformation matrices: real->complex spherical harmonics (for test)
       if(ndij==4.and.abs(pawprtvol)>=0) then
         LIBPAW_ALLOCATE(noccmmp_ylm,(2*lcur+1,2*lcur+1,ndij))
         noccmmp_ylm=czero
         LIBPAW_ALLOCATE(noccmmp_slm,(2*lcur+1,2*lcur+1,ndij))
         noccmmp_slm=czero
         LIBPAW_ALLOCATE(noccmmp_jmj,(2*(2*lcur+1),2*(2*lcur+1)))
         noccmmp_jmj=czero
...
!    ########################################################################################
     if(usepawu/=0.and.dmatudiag_loc>0) then
       ...
       LIBPAW_ALLOCATE(noccmmp_tmp,(1,ldim,ldim,ndij))
       if (ndij==4)  then
         LIBPAW_ALLOCATE(znoccmmp_tmp,(2*ldim,2*ldim))
       end if
...
!      Diagonalize nocc_mmp
       if (ndij/=4) then
         LIBPAW_ALLOCATE(hdp,(ldim,ldim,ndij))
         hdp=zero
         lwork=3*ldim-1
         LIBPAW_ALLOCATE(rwork,(lwork))
         LIBPAW_ALLOCATE(eig,(ldim))
         do ispden=1,ndij
         ...
         LIBPAW_DEALLOCATE(rwork)
         LIBPAW_DEALLOCATE(eig)
       else
         LIBPAW_ALLOCATE(hdp,(2*ldim,2*ldim,1))
         hdp=zero
         lwork=4*ldim-1
         LIBPAW_ALLOCATE(rwork,(6*ldim-2))
         LIBPAW_ALLOCATE(zwork,(lwork))
         LIBPAW_ALLOCATE(eig,(2*ldim))
...
!      Back rotation of diagonalized matrix and printing
       if(abs(pawprtvol)>=1) then
         if (ndij/=4) then
           LIBPAW_ALLOCATE(hdp2,(ldim,ldim))
           do ispden=1,ndij
           ...
           LIBPAW_DEALLOCATE(hdp2)
         else
           LIBPAW_ALLOCATE(zhdp,(2*ldim,2*ldim))
           LIBPAW_ALLOCATE(zhdp2,(2*ldim,2*ldim))
           ...
           znoccmmp_tmp=zhdp
           LIBPAW_DEALLOCATE(zhdp)
           LIBPAW_DEALLOCATE(zhdp2)
         end if
...
&         trim(dspin(ispden+2*(ndij/4)))," =="
     ...
     if (usepawu/=0.and.dmatudiag_loc>0) then
       LIBPAW_DEALLOCATE(noccmmp_tmp)
       if (ndij==4)  then
         LIBPAW_DEALLOCATE(znoccmmp_tmp)
       end if
     end if
...
!Memory deallocation
 if (usepawu/=0.and.impose_dmat/=0) then
   do iatom_tot=1,natom
     ...
     if (lpawu/=-1)  then
       LIBPAW_DEALLOCATE(tmp_noccmmp(iatom_tot)%value)
     end if
   end do
   LIBPAW_DEALLOCATE(tmp_noccmmp)
 end if
...
!  Definition of the mesh used for integration.
 ...
 LIBPAW_ALLOCATE(phiphj1,(mesh_size))
 do klmn=1,lmn2_size
...
&           (pawtab%indklmn(5,klmn)==pawtab%indklmn(5,klmn1))) then
 ...
 LIBPAW_DEALLOCATE(gg)
 LIBPAW_DEALLOCATE(ff)
 LIBPAW_DEALLOCATE(phiphj)
 LIBPAW_DEALLOCATE(phiphj1)
