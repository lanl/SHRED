...
!Final printing
 if(xmpi_comm_rank(my_comm_grid).eq.0) then
 if (paral_atom) then
 ...
 do klmn=1,lmn2_size
   ...
   do idir = 1, 3
     ...
     dijnd(2*klmn-1,1) = dijnd(2*klmn-1,1) + &
       & intgr3(kln)*real(lms)*nucdipmom(idir)*FineStructureConstant2*pawtab%dltij(klmn)
     dijnd(2*klmn,1) = dijnd(2*klmn,1) + &
       & intgr3(kln)*aimag(lms)*nucdipmom(idir)*FineStructureConstant2*pawtab%dltij(klmn)
