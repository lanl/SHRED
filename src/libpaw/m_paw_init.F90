...
&                                              *pawtab(itypat)%tphi(1:mesh_size,jln)
   ...
   if (usekden==1)  then
     ...
     if (allocated(pawtab(itypat)%nablaphi)) then
       LIBPAW_DEALLOCATE(pawtab(itypat)%nablaphi)
     end if
     LIBPAW_ALLOCATE(pawtab(itypat)%nablaphi,(pw_mesh_size,basis_size))
     if (allocated(pawtab(itypat)%tnablaphi)) then
       LIBPAW_DEALLOCATE(pawtab(itypat)%tnablaphi)
     end if
     LIBPAW_ALLOCATE(pawtab(itypat)%tnablaphi,(pw_mesh_size,basis_size))
     LIBPAW_ALLOCATE(der,(pw_mesh_size))
     do iln=1,basis_size
