...
MODULE m_paw_onsite
 ...
 use m_paw_sphharm, only : setnabla_ylm, set_x_nabla_ylm
...
!public procedures.
 ...
 public ::  paw_x_nabla_init      ! Evaluate valence-valence on-site contribs of the nabla operator in cart. coord.
 public ::  paw_dipole_init         ! Evaluate valence-valence on-site contribs of the dipole operator (-atomic position) in cart. coord.
...
!!****f* m_paw_onsite/paw_x_nabla_init
!! NAME
!! paw_x_nabla_init
!!
!! FUNCTION
...
!! SIDE EFFECTS
!!  Pawtab(ntypat) <type(pawtab_type>=PAW tabulated starting data:
!!    %has_x_nabla=set to 1 in matrix elements are calculated and stored
!!    %x_nabla_ij(3, 3,lmn_size,lmn_size)= <phi_i|x nabla|phi_j>-<tphi_i|x nabla|tphi_j>
!!
!! NOTES
!!  Addapted from nabla_ij routine
!!
!!
!! SOURCE

subroutine paw_x_nabla_init(mpsang,ntypat,pawrad,pawtab)
  ...
  !arrays
   integer, contiguous, pointer :: indlmn(:,:)
   real(dp) :: ang_phixpphj(mpsang**2,mpsang**2,24)
   real(dp),allocatable :: dphi(:),dtphi(:),ff(:),int1(:,:),int2(:,:),rad(:)
  ...
  !Integration of the angular part: all angular integrals have been computed 
  !outside Abinit and tabulated for each (l,m) value up to l=3
   call set_x_nabla_ylm(ang_phixpphj,mpsang)
  
   do itypat=1,ntypat
  
  !  COMPUTE x_nabla_ij := <phi_i|x_nabla|phi_j>-<tphi_i|x_nabla|tphi_j> for this type
     mesh_size=pawtab(itypat)%mesh_size
     lmn_size=pawtab(itypat)%lmn_size
     nln=pawtab(itypat)%basis_size
  
     if (allocated(pawtab(itypat)%x_nabla_ij)) then
       LIBPAW_DEALLOCATE(pawtab(itypat)%x_nabla_ij)
     end if
     LIBPAW_ALLOCATE(pawtab(itypat)%x_nabla_ij,(3,3,lmn_size,lmn_size))
     pawtab(itypat)%has_x_nabla=1
  ...
  !  int1=\int phi phj dr - \int tphi tphj dr
     do jln=1,nln
       do iln=1,nln
         ff(2:mesh_size)= ( &
  &       pawtab(itypat)%phi (2:mesh_size,iln)*pawtab(itypat)%phi (2:mesh_size,jln) &
  &       -pawtab(itypat)%tphi(2:mesh_size,iln)*pawtab(itypat)%tphi(2:mesh_size,jln) )
         call pawrad_deducer0(ff,mesh_size,pawrad(itypat))
         call simp_gen(intg,ff,pawrad(itypat))
         int1(iln,jln)=intg
       end do
  ...
  !  int2=\int r phi/r d/dr(phj/r) r^2dr - \int r tphi/r d/dr(tphj/r)r^2 dr
     do jln=1,nln
       ff(1:mesh_size)=pawtab(itypat)%phi(1:mesh_size,jln)
       call nderiv_gen(dphi,ff,pawrad(itypat))
       ff(1:mesh_size)=pawtab(itypat)%tphi(1:mesh_size,jln)
       ...
       do iln=1,nln
         ff(2:mesh_size)= &
  &       pawtab(itypat)%phi (2:mesh_size,iln)*dphi (2:mesh_size)*rad(2:mesh_size) &
  &       -pawtab(itypat)%phi (2:mesh_size,iln)*pawtab(itypat)%phi (2:mesh_size,jln) &
  &       -( pawtab(itypat)%tphi(2:mesh_size,iln)*dtphi(2:mesh_size)*rad(2:mesh_size) &
  &       -pawtab(itypat)%tphi(2:mesh_size,iln)*pawtab(itypat)%tphi(2:mesh_size,jln) )
         call pawrad_deducer0(ff,mesh_size,pawrad(itypat))
         call simp_gen(intg,ff,pawrad(itypat))
         int2(iln,jln)=intg
  ...
  !  Integration of the radial part, Note unpacked loop
     do jlmn=1,lmn_size
       ...
       do ilmn=1,lmn_size
         ...
         pawtab(itypat)%x_nabla_ij(1,1,ilmn,jlmn)= &
  &       int2(il,jl)* ang_phixpphj(ilm,jlm,1) &
  &       +int1(il,jl)*(ang_phixpphj(ilm,jlm,19)+ang_phixpphj(ilm,jlm,10))
  
         pawtab(itypat)%x_nabla_ij(1,2,ilmn,jlmn)= &
  &       int2(il,jl)* ang_phixpphj(ilm,jlm,4) &
  &       +int1(il,jl)*(ang_phixpphj(ilm,jlm,22)+ang_phixpphj(ilm,jlm,13))
  
         pawtab(itypat)%x_nabla_ij(1,3,ilmn,jlmn)= &
  &       int2(il,jl)* ang_phixpphj(ilm,jlm,7) &
  &       +int1(il,jl)* ang_phixpphj(ilm,jlm,16)

         pawtab(itypat)%x_nabla_ij(2,1,ilmn,jlmn)= &
  &       int2(il,jl)* ang_phixpphj(ilm,jlm,2) &
  &       +int1(il,jl)*(ang_phixpphj(ilm,jlm,20)+ang_phixpphj(ilm,jlm,11))
  
         pawtab(itypat)%x_nabla_ij(2,2,ilmn,jlmn)= &
  &       int2(il,jl)* ang_phixpphj(ilm,jlm,5) &
  &       +int1(il,jl)*(ang_phixpphj(ilm,jlm,23)+ang_phixpphj(ilm,jlm,14))
  
         pawtab(itypat)%x_nabla_ij(2,3,ilmn,jlmn)= &
  &       int2(il,jl)* ang_phixpphj(ilm,jlm,8) &
  &       +int1(il,jl)* ang_phixpphj(ilm,jlm,17)

         pawtab(itypat)%x_nabla_ij(3,1,ilmn,jlmn)= &
  &       int2(il,jl)* ang_phixpphj(ilm,jlm,3) &
  &       +int1(il,jl)*(ang_phixpphj(ilm,jlm,21)+ang_phixpphj(ilm,jlm,12))
  
         pawtab(itypat)%x_nabla_ij(3,2,ilmn,jlmn)= &
  &       int2(il,jl)* ang_phixpphj(ilm,jlm,6) &
  &       +int1(il,jl)*(ang_phixpphj(ilm,jlm,24)+ang_phixpphj(ilm,jlm,15))
  
         pawtab(itypat)%x_nabla_ij(3,3,ilmn,jlmn)= &
  &       int2(il,jl)* ang_phixpphj(ilm,jlm,9) &
  &       +int1(il,jl)* ang_phixpphj(ilm,jlm,18)
  
       end do !ilmn
     end do !jlmn
  
     pawtab(itypat)%has_x_nabla=2
     LIBPAW_DEALLOCATE(ff)
     LIBPAW_DEALLOCATE(rad)
     LIBPAW_DEALLOCATE(int2)
     LIBPAW_DEALLOCATE(int1)
  ...
  end subroutine paw_x_nabla_init
  !!***
...
!----------------------------------------------------------------------
!!****f* m_paw_onsite/paw_dipole_init
!! NAME
!! pawnabla_init
!!
!! FUNCTION
!! Evaluate all valence-valence onsite contributions of the dipole operator in cartesian coordinates.
!! Store values in the pawtab% data structure.
!!
!! INPUTS
!!  mpsang=1+maximum angular momentum
...
!!  Pawtab(ntypat) <type(pawtab_type>=PAW tabulated starting data:
!!    %has_nabla=set to 1 in matrix elements are calculated and stored
!!    %nabla_ij(3,lmn_size,lmn_size)= <phi_i|x|phi_j>-<tphi_i|x|tphi_j>
!!
!!
!! PARENTS
!!
...
subroutine paw_dipole_init(mpsang,ntypat,pawrad,pawtab)
...
!arrays
 integer, contiguous, pointer :: indlmn(:,:)
 real(dp) :: ang_phipphj(mpsang**2,mpsang**2,8)
 real(dp),allocatable :: ff(:),int1(:,:),rad(:)
...
!Integration of the angular part: all angular integrals have been computed 
!outside Abinit and tabulated for each (l,m) value up to l=3
 call setnabla_ylm(ang_phipphj,mpsang) !A subset of these are the same for dipole as nabla
...
!  COMPUTE nabla_ij := <phi_i|nabla|phi_j>-<tphi_i|nabla|tphi_j> for this type
   ...
   if (allocated(pawtab(itypat)%dipole_ij)) then
     LIBPAW_DEALLOCATE(pawtab(itypat)%dipole_ij)
   end if
   LIBPAW_ALLOCATE(pawtab(itypat)%dipole_ij,(3,lmn_size,lmn_size))
   pawtab(itypat)%has_dipole=1
...
!  int1=\int phi phj*r dr - \int tphi tphj *r dr
   do jln=1,nln
     do iln=1,nln
       ff(1:mesh_size)= ( &
&       pawtab(itypat)%phi (1:mesh_size,iln)*pawtab(itypat)%phi (1:mesh_size,jln) &
&       -pawtab(itypat)%tphi(1:mesh_size,iln)*pawtab(itypat)%tphi(1:mesh_size,jln) ) * rad(1:mesh_size)
       call simp_gen(intg,ff,pawrad(itypat))
       int1(iln,jln)=intg
     end do
   end do
...
!  Integration of the radial part, Note unpacked loop
   do jlmn=1,lmn_size
     ...
     do ilmn=1,lmn_size
       ...
       pawtab(itypat)%dipole_ij(1,ilmn,jlmn)= &
&       int1(il,jl)* ang_phipphj(ilm,jlm,1) 

       pawtab(itypat)%dipole_ij(2,ilmn,jlmn)= &
&       int1(il,jl)* ang_phipphj(ilm,jlm,4) 

       pawtab(itypat)%dipole_ij(3,ilmn,jlmn)= &
&       int1(il,jl)* ang_phipphj(ilm,jlm,7) 

     end do !ilmn
   end do !jlmn

   pawtab(itypat)%has_dipole=2
   LIBPAW_DEALLOCATE(ff)
   LIBPAW_DEALLOCATE(rad)
   LIBPAW_DEALLOCATE(int1)
...
end subroutine paw_dipole_init
