module linalg
  ...
  contains
    ...
    subroutine deigh_generalized(Am, Bm, lam, c)
      ...
      call dsygvd(1,'V','U',n,c,n,Bmt,n,lam,work,lwork,iwork,liwork,info)
      if (info /= 0) then
    ...
    subroutine deigh_simple(Am, lam, c)
      ...
      c = Am
      call dsyevd('V','U',n,c,n,lam,work,lwork,iwork,liwork,info)
      if (info /= 0) then
    ...
    subroutine zeigh_generalized(Am, Bm, lam, c)
      ...
      call zhegvd(1,'V','U',n,c,n,Bmt,n,lam,work,lwork,rwork,lrwork,iwork,liwork,info)
      if (info /= 0) then
    ...
    subroutine zeigh_simple(Am, lam, c)
      ...
      c = Am
      call zheevd("V", "U", n, c, lda, lam, work, lwork, rwork, lrwork, &
           iwork, liwork, info)
      if (info /= 0) then
    ...
    function zinv(Am) result(Bm)
      ...
      if (info /= 0) then
         ...
         print *,'inv: zgetri error'
         stop
      end if
      Bm = Amt
      deallocate(Amt, ipiv, work)
    end function zinv
    ...
    subroutine dassert_shape(A, shap, routine, matname)
      ...
      if(any(shape(A) /= shap)) then
         ...
         print *,"Aborting due to illegal matrix operation"
         stop
      end if
  ...
  integer :: info, n
  n = size(a)
  allocate(work(1 + 4*n + n**2), iwork(3 + 5*n))
  call dstevd("V", n, a, b, U, n, work, size(work), iwork, size(iwork), info)
  if (info /= 0) then
     print *, "info =", info
     print *,"info /= 0"
     stop
  ...
  subroutine zeig_hess(H, VR, VL, lam)
   complex(dp), intent(in) :: H(:,:) ! Upper Hessenberg Matrix
   complex(dp), intent(out) ::  VR(:,:), VL(:,:)
   complex(dp), intent(out) :: lam(:)

   complex(dp), allocatable :: work(:), Z(:,:)
   real(dp), allocatable ::Rwork(:)
   logical, allocatable :: select(:)
   integer, allocatable :: ifailR(:), ifailL(:)
   integer :: info, n, ldH, lwork, ldz, ldVL, ldVR
   n = size(H,2)
   if(n.lt.1) return
   ...
   allocate(work(lwork))
   allocate(Z(ldz,n))
   call  ZHSEQR( 'S', 'N', n, 1, n, H, ldH, lam, Z, ldz, &
                  work, lwork, info )
   deallocate(work)
   deallocate(Z)
   ...
   lDVL=n
   lDVR=n
   call ZHSEIN( 'B', 'Q', 'N', select, n, H, ldH, lam, VL, &
                lDVL, VR, lDVR, n, n, work, Rwork, ifailL, &
                ifailR, info)
   if(any(ifailR.gt.0)) print *, 'Right Eigenvector failed to converge in zeig_hess '
   if(any(ifailL.gt.0)) print *, 'Left Eigenvector failed to converge in zeig_hess '
