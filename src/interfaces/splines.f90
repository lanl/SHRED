...
public spline3pars, spline3valder, iix, iixmin, iixun, iixexp, poly3, dpoly3, &
    d2poly3, spline3, spline3ders, spline3_unsorted, spline3ders_unsorted

contains

function spline3_unsorted(x, y, xnew) result(ynew)
   real(dp), intent(in) :: x(:), y(:), xnew(:)
   real(dp) :: ynew(size(xnew)), x_sorted(size(xnew)),y_sorted(size(xnew))
   integer :: id(size(xnew)), i
   ...
   call Sort(x_sorted, id, 1, size(xnew))

   y_sorted = spline3(x, y, x_sorted)
   
   do i=1,size(xnew)
      ynew(id(i))=y_sorted(i)
   enddo
   
end function

subroutine spline3ders_unsorted(x, y, xnew, ynew, dynew, d2ynew)
   real(dp), intent(in) :: x(:), y(:), xnew(:)
   real(dp), intent(out) :: ynew(:), dynew(:), d2ynew(:)
   real(dp) :: x_sorted(size(xnew)), y_sorted(size(xnew),3)
   integer :: id(size(xnew)), i
   ...
   call spline3ders(x, y, x_sorted, y_sorted(:,1), y_sorted(:,2), y_sorted(:,3)) 

   do i=1,size(xnew)
      ynew(id(i))=y_sorted(i,1)
      dynew(id(i))=y_sorted(i,2)
      d2ynew(id(i))=y_sorted(i,3)
   enddo

end subroutine

 recursive subroutine Sort(x, id, left, right)
   implicit none
   real(dp),intent(inout)  :: x(:)
   integer,intent(inout) :: id(:)
   real(dp)  :: pivot, temp_x
   integer :: temp_id
   integer :: left , right
   integer :: i, j
   ...
   do
           ...
           temp_id = id(i);  id(i) = id(j);  id(j) = temp_id
   
           i=i+1; j=j-1
   end do
   if (j+1 < right)  call Sort(x, id, j+1, right)
   if (left < i-1) call Sort(x, id, left, i-1)
   end subroutine Sort
