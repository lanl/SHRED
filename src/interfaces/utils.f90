module utils
    ...
    public upcase, lowcase, whitechar, blank, num_strings, getstring, &
        stop_error, arange, loadtxt, loadtxt2, savetxt, newunit, assert, str, &
        zeros, mesh_exp, linspace, strfmt, get_int_arg, get_float_arg, &
        allocate_mold, number_of_columns
    ...
    interface allocate_mold
        ...
        module procedure allocate_mold_complex4d
    end interface
    ...
    subroutine loadtxt2(filename, d, s)
    ...
    integer, intent(in) :: s !predefined unit for write
    ...
    subroutine allocate_mold_complex4d(A, B)
    complex(dp), allocatable, intent(out) :: A(:,:,:,:)
    complex(dp), intent(in) :: B(:,:,:,:)
    ! Equivalent to allocate(A, mold=B). Use this with compilers that do not
    ! support the F2008 syntax yet.
